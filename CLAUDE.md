...
## Current Version
**v1.1: IN PROGRESS** — chat-activity-slack-1 complete (May 15, 2026). Chat sessions on the CE workspace now post a parent message to a public Slack channel on the first visitor message, with a thread reply once the AI summary is generated. Both notifications deep-link to the session in the dashboard via `?session=<id>`.
...
4. **chat_sessions** — Conversation logs. Fields: session_token (client UUID), messages (JSONB array with message_id per message), summary (JSONB), metadata (JSONB), visitor_name, visitor_email, escalated, escalated_at, slack_thread_ts (Slack `ts` of the chat-activity parent message — set when the start notification fires on the CE workspace, NULL otherwise). Unique on (workspace_id, session_token).
