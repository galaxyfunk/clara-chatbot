 ...
 *
 * This is a hand-maintained mirror of `site/src/lib/ask/brief.ts` in
 * galaxyfunk/cloudemployee.io. Keep the two in sync field-for-field; CE
 * validates incoming briefs against its own copy and ignores fields it does
 * not know, so additions are safe but renames are not.
 *
 * Every field except `intent`, `version` and `strength` is optional BY DESIGN.
 * CE draws an absent field as a dashed "Clara will ask next" prompt. A guessed
 ...
 * The complete brief goes over the wire every time, never a patch.
 */
...
/**
 * Chip row for the current question. Last item is always Other.
 * CE today only parses token / brief_update / done / error, so Clara also
 * names these options in the sentence until the website learns this event.
 */
