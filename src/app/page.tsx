...
export default function HomePage() {
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {/* Widget Layout Showcase Section */}
      <section className="py-20 px-4 sm:px-6 lg:px-8 bg-gradient-to-b from-[#213D66] to-[#1a2d4d]">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-16">
            ...
            <p className="mt-4 text-lg text-white/70 max-w-2xl mx-auto">
              Three beautiful layouts to match your brand. All with real frosted glass effects and smooth animations.
            </p>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            ...
            <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 border border-white/20">
              ...
              <h3 className="text-xl font-semibold text-white mb-2">Classic Bubble</h3>
              <p className="text-white/70 text-sm mb-4">
                The familiar floating chat bubble. Unobtrusive and always accessible.
              </p>
              <ul className="space-y-2">
                {['Floating trigger', 'Customizable position', 'Works everywhere'].map((feature) => (
                  <li key={feature} className="flex items-center gap-2 text-sm text-white/80">
                    <Check className="w-4 h-4 text-ce-lime flex-shrink-0" />
                    {feature}
                  </li>
                ))}
              </ul>
            ...
            {/* Side Whisper Card */}
            <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 border border-white/20 ring-2 ring-ce-lime/50">
              <div className="flex items-center gap-2 mb-4">
                <span className="px-2 py-1 text-xs font-bold bg-ce-lime text-ce-navy rounded">NEW</span>
              </div>
              <div className="mb-4">
                <SideWhisperMockup />
              </div>
              <h3 className="text-xl font-semibold text-white mb-2">Side Whisper</h3>
              <p className="text-white/70 text-sm mb-4">
                A sleek side panel that slides in from the right edge.
              </p>
              <ul className="space-y-2">
                {['Frosted glass design', 'Full-height panel', 'Premium feel'].map((feature) => (
                  <li key={feature} className="flex items-center gap-2 text-sm text-white/80">
                    <Check className="w-4 h-4 text-ce-lime flex-shrink-0" />
                    {feature}
                  </li>
            ...
            <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 border border-white/20 ring-2 ring-ce-lime/50">
              ...
              <h3 className="text-xl font-semibold text-white mb-2">Command Bar</h3>
              <p className="text-white/70 text-sm mb-4">
                Spotlight-style overlay for power users. Press ⌘K.
              </p>
              <ul className="space-y-2">
                {['Keyboard shortcut', 'Centered modal', 'Blurred backdrop'].map((feature) => (
                  <li key={feature} className="flex items-center gap-2 text-sm text-white/80">
                    <Check className="w-4 h-4 text-ce-lime flex-shrink-0" />
                    {feature}
                  </li>
      ...
      {/* Live Demo Section */}
      <section id="demo" className="py-20 px-4 sm:px-6 lg:px-8 bg-ce-muted">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-12">
            ...
            <h2 className="text-3xl sm:text-4xl font-bold text-ce-navy">
              Experience Clara Firsthand
            </h2>
            <p className="mt-4 text-lg text-ce-text-muted max-w-2xl mx-auto">
              Click the widget on the right to chat. Ask about Cloud Employee&apos;s services, pricing, or anything else.
            </p>
          </div>

          {/* Demo container - desktop only */}
          <div className="hidden md:block relative min-h-[550px] bg-white rounded-2xl shadow-xl overflow-hidden border border-ce-border">
            {/* Simulated website content */}
            <div className="p-8">
              <div className="flex items-center gap-3 mb-8">
                <div className="w-10 h-10 rounded-full bg-[#213D66] flex items-center justify-center">
                  <span className="text-white font-bold">CE</span>
                </div>
                <span className="font-semibold text-lg text-ce-navy">Cloud Employee</span>
              </div>
              <h3 className="text-3xl font-bold text-gray-800 mb-4">
                Welcome to our demo site
              </h3>
              <p className="text-gray-600 mb-6 max-w-lg">
                This is a demonstration of how Clara looks embedded on your website.
                Click the &quot;Ask Clara&quot; tab on the right edge to start chatting!
              </p>
              ...
              <div className="mt-8 flex gap-4">
                <div className="px-6 py-3 bg-[#C5E84D] text-[#213D66] font-semibold rounded-lg">
                  Book a Call
                </div>
                <div className="px-6 py-3 border-2 border-[#213D66] text-[#213D66] font-semibold rounded-lg">
                  Learn More
                </div>
              </div>
            </div>

            {/* The actual Clara widget */}
            <Script
              src="/widget.js"
              data-workspace-id="09aa62df-5af6-4cec-b565-c335e907327d"
              strategy="lazyOnload"
            />
          </div>

          {/* Mobile fallback */}
          <div className="md:hidden text-center py-12 bg-white rounded-2xl border border-ce-border">
            <div className="w-16 h-16 bg-ce-muted rounded-full flex items-center justify-center mx-auto mb-4">
              <MessageSquare className="w-8 h-8 text-ce-teal" />
            </div>
            <p className="text-ce-text-muted px-6">
              View on desktop to try the live demo widget with full frosted glass effects.
            </p>
          </div>

          <p className="mt-6 text-center text-sm text-ce-text-muted">
            This demo uses the Side Whisper layout. Sign up to try all three styles!
          </p>
        </div>
      </section>
