...
import { isVagueRole, type IntakeSnapshot } from '@/lib/chat/intake-plan';
import type {
  Brief,
  BriefContent,
  BriefEngagement,
...
The first visitor message may be labelled "[The visitor did not type this...]". Those are known facts from their booking page. Treat them as visitor-stated. job_title is who they are at the company. It is NEVER the role they are hiring.

Map extra intake answers into the fields we already have. Do not invent new keys.
- How they identify, company size, B2B/B2C → companyContext
- Who manages the hire, remote habit, tools, how documented the work is, management style → teamContext
- What good looks like in the person, 90-day success, volunteered budget → mustHaves / goals
- Enterprise compliance (SOC2, healthcare, finance) → complianceFlags
- Agency project vs a seat they want to keep → engagement
Watch-outs are sentences inside teamContext or goals, never a score: HR is booking (not the hiring manager), role still generic, no manager named, first overseas hire, almost no docs, agency short project.

Never write "vibe coder". Never copy the visitor's own job title into roles.
...
/**
 * Named facts from the /ask seed, as a Brief the canvas can paint before
 * anyone types. job_title never becomes a role.
 */
export function briefFromIntakeSnapshot(snapshot: IntakeSnapshot): Brief {
  const content: BriefContent = {
    intent: 'unknown',
    strength: 0,
  };

  const headcount = parseHeadcount(snapshot.named.headcount);
  if (headcount !== undefined) content.headcount = headcount;

  if (snapshot.named.role) {
    const role: BriefRole = {
      title: snapshot.named.role,
      count: headcount ?? 1,
    };
    const seniority = seniorityFromTitle(snapshot.named.role);
    if (seniority) role.seniority = seniority;
    if (snapshot.named.stack?.length) role.stacks = snapshot.named.stack;
    content.roles = [role];
    content.intent = (headcount ?? 1) > 1 ? 'team_hire' : 'single_hire';
  } else if ((headcount ?? 0) > 1) {
    content.intent = 'team_hire';
  }

  if (snapshot.named.stack?.length) content.techStacks = snapshot.named.stack;
  if (snapshot.named.timeline) content.timeline = snapshot.named.timeline;

  const companyBits: string[] = [];
  if (snapshot.named.company) companyBits.push(snapshot.named.company);
  if (snapshot.identity) companyBits.push(snapshot.identity);
  if (companyBits.length) content.companyContext = companyBits.join('. ');

  const teamBits: string[] = [];
  if (snapshot.buyer_looks_hr) {
    teamBits.push('Buyer is HR, not the hiring manager. Confirm who the person reports to.');
  }
  if (isVagueRole(snapshot.named.role)) {
    teamBits.push('Role is still generic. Shortlist will be a guess until the call.');
  }
  if (teamBits.length) content.teamContext = teamBits.join(' ');

  content.strength = computeBriefStrength(content);
  return { version: 1, ...content };
}

function parseHeadcount(value?: string): number | undefined {
  if (!value) return undefined;
  const n = parseInt(value.replace(/[^\d]/g, ''), 10);
  if (!Number.isFinite(n) || n < 1 || n > MAX_PEOPLE) return undefined;
  return n;
}

function seniorityFromTitle(title: string): string | undefined {
  const match = title.trim().match(/^(junior|mid-level|mid|senior|staff|principal|lead)\b/i);
  if (!match) return undefined;
  const word = match[1].toLowerCase();
  if (word === 'mid') return 'Mid-level';
...
export async function persistIntakeSnapshot(
  ...
  if (readError) {
    console.error('[Intake] Could not read metadata before persisting:', readError.message);
    return;
  }
  ...
  const { error: writeError } = await supabase
    .from('chat_sessions')
    .update({ metadata: { ...metadata, intake: snapshot } })
    .eq('id', sessionId);

  if (writeError) {
    console.error('[Intake] Persist failed:', writeError.message);
  }
}
